...
/* FreeRTOS 事件组，用于指示我们何时连接成功 */
static EventGroupHandle_t s_wifi_event_group;

/* 事件组允许每个事件包含多个位，但我们就只关心两个事件：
 * - 我们已连接到 AP 并获取了 IP
 * - 我们在重试最大次数后连接失败 */
...
void app_wifi_init(void)
{
    ...
    wifi_config_t wifi_config = {
        .sta = {
            ...
            /* 如果密码符合 WPA2 标准（密码长度 => 8），则认证模式阈值默认为 WPA2。
             * 如果您想连接到已弃用的 WEP/WPA 网络，请将阈值设置为
             * WIFI_AUTH_WEP/WIFI_AUTH_WPA_PSK，并设置符合
             * WIFI_AUTH_WEP/WIFI_AUTH_WPA_PSK 标准的密码长度和格式。
             */
...
{
    /* 等待直到连接建立 (WIFI_CONNECTED_BIT) 或连接失败达到最大重试次数 (WIFI_FAIL_BIT)。
    ...
    /* xEventGroupWaitBits() 返回调用前的位，因此我们可以测试实际发生了哪个事件。 */
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(TAG, "已连接到 AP SSID:%s password:%s",
                 EXAMPLE_ESP_WIFI_SSID, EXAMPLE_ESP_WIFI_PASS);
        return ESP_OK;
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI(TAG, "连接失败 SSID:%s, password:%s",
                 EXAMPLE_ESP_WIFI_SSID, EXAMPLE_ESP_WIFI_PASS);
        return ESP_FAIL;
    } else {
        ESP_LOGE(TAG, "意外事件");

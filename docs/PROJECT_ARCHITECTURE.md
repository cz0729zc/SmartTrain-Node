...
参考资料：<https://blog.csdn.net/ik67890123/article/details/155567881>

核心思想摘要（已映射到本项目执行规则）：

- 不要“只看现象”，必须同时观察 CPU、内存、中断、调度四个维度。
- 先建立监测，再谈优化；没有数据支撑的“优化”只是在碰运气。
- 优先定位高频路径与高优先级路径，尤其是 ISR、WiFi、UI 刷新、队列等待和内存分配。
- 任何性能回归都应被当作工程问题管理，而不是临时修补。

#### 4.6.1 推荐的调试工具链

| 工具 / 方法                 | 作用                                                         |
|-----------------------------|--------------------------------------------------------------|
| `vTaskList()`               | 快速查看任务状态、优先级、栈占用                           |
| `uxTaskGetStackHighWaterMark()` | 观察任务栈水位，预防栈溢出                               |
| `vTaskGetRunTimeStats()`    | 统计各任务 CPU 占用分布                                     |
| `heap_caps_get_free_size()` / `heap_caps_dump_all()` | 观察各内存池剩余和碎片化情况 |
| `perfmon` / 运行时统计      | 分析双核负载、热点任务和长尾抖动                             |
| GPIO 翻转 + 逻辑分析仪       | 测量关键函数、ISR 和外设访问时序                             |
| 示波器/电流探头             | 对齐功耗峰值与日志时间戳，定位外设激活窗口                   |

#### 4.6.2 调试准则

1. 先判断问题属于哪一类：CPU、内存、中断、调度、外设或协议栈。
2. 先做最小复现，逐步增加模块，不要同时改多项变量。
3. 对“可能阻塞”的路径做边界检查：串口读写、SPI 传输、队列等待、事件等待、动态内存申请。
4. ISR 内只做轻量通知，禁止在中断里执行日志、分配内存、复杂计算或长时间轮询。
5. 高频任务优先绑定核心，避免双核之间随机迁移。
6. 动态分配要监控最大空闲块与碎片化，不要只看总空闲内存。

#### 4.6.3 常见性能反模式

- `double` 替代 `float`，在单精度路径上引入软浮点开销。
- 用 `vTaskDelay()` 代替事件驱动，造成无意义轮询。
- 在 ISR 或高优先级回调里调用 Flash 常规函数，增加中断延迟。
- 将高频回调放在 PSRAM 或把关键缓冲错误地放到非 DMA 内存。
- 任务不绑定核心，导致热点任务集中在同一核上。

#### 4.6.4 本项目落地要求

- WiFi / MQTT / 网络维护任务固定到 Core 0。
- UI 刷新、触摸和本地交互固定到 Core 1。
- 触摸轮询、LCD 刷新和事件处理必须区分“测试路径”和“常规路径”，避免把调试逻辑带进正式流程。
- 遇到重启或抖动，先输出任务表、栈水位、堆池快照，再改代码。

#### 4.6.5 性能回归治理

- 给关键版本建立基线：CPU 占用、最大栈深、最大中断延迟、空闲堆、PSRAM 使用量。
- 任何后续优化都要可量化回归比较。
- 对反复出现的问题建立“反模式”条目，写入项目文档，避免重复踩坑。
